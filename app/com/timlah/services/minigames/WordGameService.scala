...
class WordGameService {
  ...
  def buildWordObject(submission: String): Unit = {
    ...
    val submissionCounts                       =
      scala.collection.mutable.Map[Char, Int]().withDefaultValue(0)
    val selectedWordCounts =
      SelectedWord.groupBy(identity).view.mapValues(_.length).toMap

    submission.toUpperCase().zipWithIndex.foreach { case (character, index) =>
      submissionCounts(character) += 1

      if (SelectedWord.charAt(index) == character) {
        characterObjects =
          characterObjects.appended(CharacterObject(character, 0))
      } else {
        if (SelectedWord.contains(character)) {
          // Check if the character has been used more than it exists in the SelectedWord
          if (
            submissionCounts(character) <= selectedWordCounts.getOrElse(
              character,
              0
            )
          ) {
            characterObjects =
              characterObjects.appended(CharacterObject(character, 1))
          } else {
            characterObjects =
              characterObjects.appended(CharacterObject(character, 2))
          }
        } else {
          characterObjects =
            characterObjects.appended(CharacterObject(character, 2))
    ...
    characterObjects.zipWithIndex.foreach {
      case (CharacterObject(character, status), index) if status == 1 =>
        if (
          ...
          characterObjects =
            characterObjects.updated(index, CharacterObject(character, 2))
          submissionCounts(character) -= 1
        }
      case _ =>
    }

    val areAllCharactersInTheCorrectPlace = characterObjects.forall {
      case CharacterObject(_, int) => int == 0
    }
    if (areAllCharactersInTheCorrectPlace) {
      guessedWordObjects =
        guessedWordObjects.appended(WordObject(characterObjects, 0))
    } else {
      guessedWordObjects =
        guessedWordObjects.appended(WordObject(characterObjects, 1))
    }
  }
