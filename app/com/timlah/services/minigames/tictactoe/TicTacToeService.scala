...
class TicTacToeService {
  ...
  def updateBoard(playerPiecePosition: Int) = {
    ...
    val updatedLayout = currentBoard.boardLayout.updated(
      playerPiecePosition - 1,
      TicTacToeSquare(playerPiecePosition, Some(playerPiece))
    )
    ...
    if (
      currentBoard.boardLayout
        .filter(_.state == Some(TicTacToePiece.NoPiece))
        .length == 0 && !hasLost && !hasWon
    ) {
      isDraw = true
    }
  }
  ...
  def botDecision() = {
    val currentBoard = board.get
    val emptySquares =
      currentBoard.boardLayout.filter(_.state == Some(TicTacToePiece.NoPiece))
    if (emptySquares.length > 1) {
      val randomSquare = emptySquares(Random.nextInt(emptySquares.size))

      val updatedLayout = currentBoard.boardLayout.updated(
        randomSquare.id - 1,
        TicTacToeSquare(randomSquare.id, Some(opponentPiece))
      )
      val updatedBoard = currentBoard.copy(boardLayout = updatedLayout)
      board = Some(updatedBoard)
    } else {
  ...
  def checkForWinCondition(): Boolean = {
    val oPieces =
      board.get.boardLayout.filter(_.state == Some(TicTacToePiece.OPiece))
    val xPieces =
      board.get.boardLayout.filter(_.state == Some(TicTacToePiece.XPiece))

    if (hasWon(oPieces, winConditions)) {
      if (playerPiece == TicTacToePiece.OPiece) { hasWon = true }
      else { hasLost = true }
    }
    if (hasWon(xPieces, winConditions)) {
      if (playerPiece == TicTacToePiece.XPiece) { hasWon = true }
      else { hasLost = true }
    }
    hasWon || hasLost
  }
  ...
  def hasWon(
      pieces: Seq[TicTacToeSquare],
      winConditions: Seq[(Int, Int, Int)]
  ): Boolean = {
    val occupiedIds = pieces.map(_.id)

    winConditions.exists { case (a, b, c) =>
      occupiedIds.contains(a) && occupiedIds.contains(b) && occupiedIds
        .contains(c)
    }
  }
