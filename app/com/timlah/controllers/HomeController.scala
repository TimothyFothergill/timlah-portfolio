...
import com.timlah.services.{
  CurrentProjects,
  EmailService,
  GravatarProfileService,
  MarkupService
}
...
class HomeController @Inject() (
  ...
  def projects() = Action { implicit request: Request[AnyContent] =>
    {
      ...
      Ok(
        com.timlah.views.html.projects(
          currentProjects.currentProjects(),
          currentProjects.closedProjects()
        )
      )
    }
  }
  ...
  def blogBySlug(slug: String) = Action.async {
    implicit request: Request[AnyContent] =>
      ...
      getFutureBlogPost.map(i =>
        Ok(
          com.timlah.views.html.blog(
            i.get,
            markupService.markdownStringToHTML(i.get.content) match {
  ...
  def generateBlogMetaTags(latest: BlogPost, blogContent: String): Html = {
    val regexMatchesImage = """!\[.*]\((.*)\)""".r
      .findFirstMatchIn(latest.content)
      .map(_.group(1))
      .getOrElse("")
    Html(s"""<meta property='og:description' content='${blogContent
        .split("\n")
        .headOption
        .getOrElse("")}'>
        <meta property='og:title' content='${latest.title}'>
        <meta property='og:image' content='${regexMatchesImage}'>
        <meta property="og:url" content="https://www.timlah.com${com.timlah.controllers.routes.HomeController
        .blogBySlug(latest.slug)}">
    ...
    implicit request: MessagesRequest[AnyContent] =>
      {
        ...
        boundForm.fold(
          formWithErrors => {
            BadRequest(
              com.timlah.views.html
                .contact(formWithErrors, enquiryTypes.map(_.asString))
            )
          },
          submittedData => {
            ...
            if (
              ...
              Redirect(routes.HomeController.index())
                .flashing("fail" -> "Contact message not sent.")
            } else {
              ...
              Redirect(routes.HomeController.index())
                .flashing("success" -> "Contact message sent, thank you.")
            }
          }
        )
      }
  ...
  def isCookieExpired(cookie: Cookie): Boolean = {
    getCookieExpiry(cookie) match {
      case Some(expiryTime) => {
        if (expiryTime.isBefore(ZonedDateTime.now())) { true }
        else { false }
      }
      case None => false
    }
    ...
    implicit request: MessagesRequest[AnyContent] =>
      ...
      boundForm.fold(
        formWithErrors => {
          BadRequest(
            com.timlah.views.html.games
              .wordgame(formWithErrors, minigameWordGameService)
          )
        },
        submittedData => {
          val guessedWord =
